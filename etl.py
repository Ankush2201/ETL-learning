...
class NewsETL(ETLPipeline):
    ...
    for month in months:
        ...
        # Temporary list to hold data for the current month
        data = []

        for news in response_json['response']['docs']:
            try:
                # Check if news desk is in the important fields
                ...
                if news.get('news_desk') in IMPORTANT_FIELDS:  # Use get to avoid KeyError
                    keywords = [i['value'] for i in news.get('keywords', [])]
        ...
        # Create a DataFrame from the collected data for the current month
        if data:  # Only create a DataFrame if there's data
            sub_df = pd.DataFrame(data, columns=['pub_date', 'snippet', 'headline', 'keywords'])
            data_frames.append(sub_df)  # Append the DataFrame to the list

    # Concatenate all DataFrames in the list into a single DataFrame
    if data_frames:
        self.df = pd.concat(data_frames, ignore_index=True)
    else:
        self.df = pd.DataFrame()  # Handle case where no data was found
